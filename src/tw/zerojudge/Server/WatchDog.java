...
public class WatchDog implements Runnable {
	...
	public void run() {
		try {
			if (this.timelimit != 0) {
				this.max_timelimit = this.timelimit * 2 + 2; // at least 2s, shell 最小單位 1s
			}
		...
		if (!this.isFinish) {
			...
			cause.setPlainMessage(
					"process 沒有結束，因此強迫結束 do process.destroy(), " + "watch 經過 " + this.max_timelimit + " s 啟動，強制結束");
			System.out.println("WatchDog:" + cause.getPlainMessage());
			return;
		}
	}
