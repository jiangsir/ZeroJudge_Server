...
public class ApplicationScope {
	...
	public static void setAllAttributes(ServletContext servletContext) {
		...
		ApplicationScope.setServerConfigFile(
				new File(ApplicationScope.getAppRoot() + "/WEB-INF/", "ServerConfig.xml"));
	 ...
	 * 取得目前系統的版本。
	 */
	public static String getVersion() throws DataException {
	 ...
	 * 取得目前系統的版本。
	 */
	public static void setVersion() throws DataException {
		try {
			ApplicationScope.version = FileUtils.readFileToString(new File(
					ApplicationScope.getAppRoot() + File.separator + "META-INF", "Version.txt"))
					.trim();
		} catch (IOException e) {
			e.printStackTrace();
