...
public class ServerConfig extends Config {
	public static String lxc_NAME = "lxc-ALL";
	...
	private String Lxc_EXEC = "sudo lxc-attach -n " + lxc_NAME;
	// sudo lxc-attach -n
	// sudo lxc exec lxd-ALL
	// @Property(key = "Lxc_NAME")
	// private String Lxc_NAME = "lxc-ALL";
	@Property(key = "Lxc_PATH")
	private String Lxc_PATH = "/var/lib/lxc/" + lxc_NAME + "/rootfs/";
	@Property(key = "JVM_MB")
	...
	@Property(key = "isTLEbreak")
	private boolean isTLEbreak = true; // 預設 true 也就是遇到一個 TLE 就中斷。
	...
	public void setInitComilers() {
		...
		c.setSuffix("c");
		c.setPath("");
		c.setVersion("gcc -std=c11 (gcc 7.4.0)");
		c.setSamplecode(
				"#include<stdio.h>\r\nint main() {\r\n char s[9999];\r\nwhile( scanf(\"%s\",s)!=EOF ) {\r\n printf(\"hello, %s\\n\",s);\r\n }\r\n return 0;\r\n}");
		c.setCommand_begin("");
		...
		cpp.setSuffix("cpp");
		cpp.setPath("");
		cpp.setVersion("g++ -std=c++14 (g++ 7.4.0)");
		cpp.setSamplecode(
				"#include <iostream>\r\nusing namespace std;\r\n\r\nint main() {\r\nstring s;\r\n while(cin >> s){\r\ncout << \"hello, \"<< s << endl;\r\n }\r\n return 0;\r\n}");
		...
		cpp.setCommand_end("");
		cpp.setRestrictedfunctions(
				new String[] { "system", "fopen", "fclose", "freopen", "fflush", "fstream", "ifstream", "ofstream",
						"time.h", "#pargma", "conio.h", "fork", "popen", "execl", "execlp", "execle", "execv", "execvp",
						"getenv", "putenv", "setenv", "unsetenv", "socket", "connect", "fwrite", "gethostbyname" });
		Compiler java = new Compiler();
		java.setEnable(KNOWNED_LANGUAGE.JAVA.name());
		java.setLanguage(KNOWNED_LANGUAGE.JAVA.name());
		java.setSuffix("java");
		java.setPath("");
		java.setVersion("OpenJDK java version 1.8.0_222");
		java.setSamplecode(
		...
		python.setSuffix("py");
		python.setPath("");
		python.setVersion("Python 3.6.9");
		python.setSamplecode("while True:\n" + "    try:\n" + "        s = input()\n" + "        print('hello, '+s)\n"
				+ "    except:\n" + "        break\n" + "");
		python.setCommand_begin("");
		python.setCmd_compile("");
		python.setCmd_namemangling("");
		python.setCmd_execute("PYTHONIOENCODING=utf-8 python3 $S/$C.py < $T > $S/$C.out");
	...
	@JsonIgnore
	public Boolean getIsTLEbreak() {
		return isTLEbreak;
	}

	@JsonIgnore
	public void setIsTLEbreak(Boolean isTLEbreak) {
		this.isTLEbreak = isTLEbreak;
	}

	@JsonIgnore
	public void setIsTLEbreak(String isTLEbreak) {
		if (isTLEbreak == null || "".equals(isTLEbreak.trim())) {
			return;
		}
		this.setIsTLEbreak(Boolean.valueOf(isTLEbreak.trim()));
	}
