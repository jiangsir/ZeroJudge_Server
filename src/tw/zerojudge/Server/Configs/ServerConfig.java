...
public class ServerConfig extends Config {
	...
	@Property(key = "Lxc_EXEC")
	private String Lxc_EXEC = "sudo lxc-attach -n lxc-ALL";
	// sudo lxc-attach -n
	// sudo lxc exec lxd-ALL
//	@Property(key = "Lxc_NAME")
//	private String Lxc_NAME = "lxc-ALL";
	@Property(key = "Lxc_PATH")
	private String Lxc_PATH = "/var/lib/lxc/lxc-ALL/rootfs/";
	@Property(key = "JVM_MB")
	...
	public void setInitComilers() {
		...
		c.setSuffix("c");
		c.setPath("");
		c.setVersion("gcc -std=c11(gcc 7.3.0)");
		c.setSamplecode(
				"#include<stdio.h>\r\nint main() {\r\n char s[9999];\r\nwhile( scanf(\"%s\",s)!=EOF ) {\r\n printf(\"hello, %s\\n\",s);\r\n }\r\n return 0;\r\n}");
		c.setCommand_begin("");
		...
		c.setCommand_end("");
		c.setRestrictedfunctions(new String[] { "system", "fopen", "fclose", "freopen", "fflush",
				"fstream", "time.h", "#pargma", "conio.h", "fork", "popen", "execl", "execlp",
				"execle", "execv", "execvp", "getenv", "putenv", "setenv", "unsetenv", "socket",
				"connect", "fwrite", "gethostbyname" });
		Compiler cpp = new Compiler();
		cpp.setEnable(KNOWNED_LANGUAGE.CPP.name());
		cpp.setLanguage(KNOWNED_LANGUAGE.CPP.name());
		cpp.setSuffix("cpp");
		cpp.setPath("");
		cpp.setVersion("g++ -std=c++14(g++ 7.3.0)");
		cpp.setSamplecode(
				"#include <iostream>\r\nusing namespace std;\r\n\r\nint main() {\r\nstring s;\r\n while(cin >> s){\r\ncout << \"hello, \"<< s << endl;\r\n }\r\n return 0;\r\n}");
		cpp.setCommand_begin("");
		cpp.setCmd_compile(
				"g++ -std=c++14 -lm -lcrypt -O2 -pipe -DONLINE_JUDGE -o $S/$C.exe $S/$C.cpp");
		...
		cpp.setCommand_end("");
		cpp.setRestrictedfunctions(new String[] { "system", "fopen", "fclose", "freopen", "fflush",
				"fstream", "ifstream", "ofstream", "time.h", "#pargma", "conio.h", "fork", "popen",
				"execl", "execlp", "execle", "execv", "execvp", "getenv", "putenv", "setenv",
				"unsetenv", "socket", "connect", "fwrite", "gethostbyname" });
		...
		java.setCommand_end("");
		java.setRestrictedfunctions(new String[] { "java\\.io\\.File.*", "java\\.net\\..*",
				"java\\.lang\\.Thread", "java\\.lang\\.Runtime", "java\\.lang\\.Runnable",
				"java\\.lang\\.Process", "java\\.applet\\..*", "java\\.awt\\..*", "java\\.nio\\..*",
				"java\\.sql\\..*", "java\\.security\\..*", "java\\.rmi\\..*",
		...
		pascal.setPath("");
		pascal.setVersion("Free Pascal Compiler version 3.0.4");
		...
		pascal.setRestrictedfunctions(new String[] { "SYSTEM_RESET", "SYSTEM_CLOSE",
				"SYSTEM_ASSIGN", "SYSTEM_REWRITE", "dos", "SysUtils", "exec", "stdcall", "external",
				"assign", "reset", "rewrite", "close", "execute", "OBJPAS_CLOSEFILE",
				"OBJPAS_ASSIGNFILE", "GenerateInstruction_CALL_FAR", "SysProc_SeekFile",
		...
		python.setSuffix("py");
		python.setPath("");
		python.setVersion("Python 3.6.6");
	...
	public void setCompilers(String compilers) throws DataException {
		...
		try {
			...
			e.printStackTrace();
			throw new DataException(ApplicationScope.getServerConfigFile().getPath()
	...
	@JsonIgnore
	public String getLxc_EXEC() {
		return Lxc_EXEC;
	}

	@JsonIgnore
	public void setLxc_EXEC(String lxc_EXEC) {
		if (lxc_EXEC == null || "".equals(lxc_EXEC.trim())) {
			return;
		}
		Lxc_EXEC = lxc_EXEC.trim();
	}

	@JsonIgnore
	public String getLxc_PATH() {
		return Lxc_PATH;
	}

	@JsonIgnore
	public void setLxc_PATH(String lxc_PATH) {
		if (lxc_PATH == null || "".equals(lxc_PATH.trim())) {
			return;
		}
		Lxc_PATH = lxc_PATH;
	}
...
//		if (lxc_NAME == null || "".equals(lxc_NAME.trim())) {
//			return;
//		}
//		Lxc_NAME = lxc_NAME.trim();
//	}
	...
	public String getVersion() {
		try {
			return ApplicationScope.getVersion();
	...
	public void setVersion(String version) {
		try {
			this.version = ApplicationScope.getVersion();
		} catch (DataException e) {
			e.printStackTrace();
