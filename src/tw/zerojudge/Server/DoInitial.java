		...
		for (int i = 0; i < serverInput.getTestfiles().length; i++) {
			if (serverInput.getPriority() == ServerInput.PRIORITY.Testjudge) {
				...
				if (!serverConfig.getTestdataPath().exists()) {
					...
					FileTools.forceMkdir(serverConfig.getTestdataPath(), serverConfig.getRsyncAccount());
				}
				...
				try {
					...
					System.out.println("serverInput.getTestjudge_indata()=" + serverInput.getTestjudge_indata());
					String[] testjudge_indatas = mapper.readValue(serverInput.getTestjudge_indata(), String[].class);
					FileTools.writeStringToFile(infile, testjudge_indatas[i], serverConfig.getRsyncAccount());
					new RunCommand("sudo chmod a+r " + infile.getAbsolutePath()).run();

					File outfile = new File(serverConfig.getTestdataPath(), serverInput.getTestfiles()[i] + ".out");
					String[] testjudge_outdatas = mapper.readValue(serverInput.getTestjudge_outdata(), String[].class);
					FileTools.writeStringToFile(outfile, testjudge_outdatas[i], serverConfig.getRsyncAccount());
					new RunCommand("sudo chmod a+r " + outfile.getAbsolutePath()).run();
				} catch (IOException e) {
					e.printStackTrace();
					output.setJudgement(ServerOutput.JUDGEMENT.SE);
			...
			String rsyncaccount = serverConfig.getRsyncAccount();

			new RunCommand("sudo chown -R " + rsyncaccount + " " + infile.getParentFile().getAbsolutePath()).run();
			new RunCommand("sudo chmod g+rwx " + outfile.getParentFile().getAbsolutePath()).run();
			new RunCommand("sudo dos2unix " + infile.getPath()).run();
			new RunCommand("sudo dos2unix " + outfile.getPath()).run();
