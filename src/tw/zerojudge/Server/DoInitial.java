	...
	public void run() throws JudgeException {
		...
		for (int i = 0; i < serverInput.getTestfiles().length; i++) {
			if (serverInput.getPriority() == ServerInput.PRIORITY.Testjudge) {
				System.out.println(
						"serverConfig.getTestdataPath()=" + serverConfig.getTestdataPath());
				if (!serverConfig.getTestdataPath().exists()) {
					// serverConfig.getTestdataPath().mkdir();
					FileTools.forceMkdir(serverConfig.getTestdataPath(),
							serverConfig.getRsyncAccount());
				}
				...
				try {
					...
					FileTools.writeStringToFile(infile, serverInput.getTestjudge_indata(),
							serverConfig.getRsyncAccount());
					new RunCommand("sudo chmod a+r " + infile.getAbsolutePath()).run();

					File outfile = new File(serverConfig.getTestdataPath(),
							serverInput.getTestfiles()[i] + ".out");
					FileTools.writeStringToFile(outfile, serverInput.getTestjudge_outdata(),
							serverConfig.getRsyncAccount());
			...
			new RunCommand("sudo chown -R " + rsyncaccount + " "
					+ infile.getParentFile().getAbsolutePath()).run();
