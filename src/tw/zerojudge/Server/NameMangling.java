		...
		if (serverInput.getCompiler().getLanguage().equals(KNOWNED_LANGUAGE.JAVA.name())) {
			...
			while (it.hasNext()) {
				...
				for (String function : restricted_functions) {
					if (nmline.split(" ")[2].toLowerCase()
							.contains(function.trim().toLowerCase())) {
		...
		for (String line : nm.getOutputStream()) {
			...
			if (line.contains("=") && line.split("=").length > 1) {
				...
				if (constant_pool.startsWith("Class")) {
					constant_pool = constant_pool.substring(constant_pool.indexOf("//") + 2).trim()
							.replaceAll("/", ".");
		...
		for (String line : nm.getOutputStream()) {
			...
			if (line.startsWith("const")) {
				...
				if (classname.startsWith("class")) {
					classname = classname.substring(classname.indexOf("//") + 2).trim()
							.replaceAll("/", ".");
					for (String rfunctions : compiler.getRestrictedfunctions()) {
						rfunctions = rfunctions.trim();
