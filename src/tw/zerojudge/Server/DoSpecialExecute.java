...
public class DoSpecialExecute {
	...
	public ExecuteOutput run() throws JudgeException {
		...
		logger.info("SpecialExecute executeInput.getCommand: " + executeInput.getCommand());
	...
	private ExecuteOutput getResult(RunCommand special_execute, ExecuteOutput output, ServerInput serverInput)
		...
		for (int i = 0; i < linecount; i++) {
			...
			logger.info("1. USEROUT=" + USEROUT + ", SYSOUT=" + SYSTEMOUT);
		}
		...
		if (ServerOutput.JUDGEMENT.AC == RESULT) {
			...
			logger.info("2. USEROUT=" + USEROUT + ", SYSTEMOUT=" + SYSTEMOUT + ", detailVisible="
					+ serverInput.getErrmsg_visible());
			if (!"".equals(USEROUT) && serverInput.getErrmsg_visible()) {
				output.setHint(output.getHint() + "您的答案為：" + USEROUT);
			}
			if (!"".equals(SYSTEMOUT) && serverInput.getErrmsg_visible()) {
	...
	private ExecuteOutput getRusage(RunCommand special_execute, ExecuteOutput output) throws JudgeException {
		...
		if (rusage.getTime() >= 0) {
			...
			logger.info("SpecialExecute rusage.timeusage: " + timeusage);
		}
